...
class OvertimeApprovalController extends OvertimeController
{
    public function index(Request $request)
    {
        $user = $request->user();
        abort_unless($user->isSuperuser() || ($user->isAdmin() && $user->jabatan === Lembur::LEVEL2_JABATAN), 403);
        $query = Lembur::with(['user.profile', 'department'])->where('status', 'Pending Approval');
        if ($user->isSuperuser()) {
            $query->where('approval_level', 1)->whereHas('user.profile', fn ($q) => $q->where('atasan_id', $user->id));
        } else {
            $query->where('approval_level', 2);
        }
        $records = $query->orderBy('tanggal_lembur')->orderBy('id')->paginate(20);
    ...
    private function decide(Request $request, int $id, bool $reject)
    {
        return DB::transaction(function () use ($request, $id, $reject) {
            $record = Lembur::lockForUpdate()->findOrFail($id);
            $user = $request->user();
            $manager = $user->isSuperuser() && $record->isValidAtasan($user);
            $hr = $user->isAdmin() && $user->jabatan === Lembur::LEVEL2_JABATAN;
