...
class LoanApprovalController extends LoanNoteController
{
    public function index(Request $request)
    {
        ...
        $records = Kasbon::with(['company', 'user.profile'])
    ...
    private function decide(Request $request, int $id, bool $reject)
    {
        return DB::transaction(function () use ($request, $id, $reject) {
            $record = Kasbon::lockForUpdate()->findOrFail($id);
            $user = $request->user();
            $manager = $user->isSuperuser() && $record->isValidAtasan($user);
            $finance = $user->jabatan === Kasbon::LEVEL2_JABATAN && $record->user_id !== $user->id;
