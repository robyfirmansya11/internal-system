...
class ExpenseApprovalController extends ExpenseReimbursementController
{
    public function index(Request $request)
    {
        ...
        $records = NotaPenggantianBiaya::with(['company', 'user.profile', 'details'])
    ...
    private function decide(Request $request, int $id, bool $reject)
    {
        return DB::transaction(function () use ($request, $id, $reject) {
            $record = NotaPenggantianBiaya::lockForUpdate()->findOrFail($id);
            $user = $request->user();
            $manager = $user->isSuperuser() && $record->isValidAtasan($user);
            $finance = $user->jabatan === NotaPenggantianBiaya::LEVEL2_JABATAN && $record->user_id !== $user->id;
