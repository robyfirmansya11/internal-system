...
class TravelApprovalController extends TravelReimbursementController
{
    public function index(Request $request)
    {
        $user = $request->user();
        $finance = $user->jabatan === PerjalananDinas::LEVEL2_JABATAN;
        abort_unless($user->isSuperuser() || $finance, 403);
        $records = PerjalananDinas::with(['company', 'user.profile', 'details'])
    ...
    private function decide(Request $request, int $id, bool $reject)
    {
        return DB::transaction(function () use ($request, $id, $reject) {
            $record = PerjalananDinas::lockForUpdate()->findOrFail($id);
            $user = $request->user();
            $manager = $user->isSuperuser() && $record->isValidAtasan($user);
            $finance = $user->jabatan === PerjalananDinas::LEVEL2_JABATAN && $record->user_id !== $user->id;
