...
class TravelApprovalApiTest extends TestCase
{
    use RefreshDatabase;
    ...
    private function permit(User $manager): PerjalananDinas
    {
        ...
        return PerjalananDinas::create([
            ...
            'keterangan' => 'Business trip', 'total' => '415.50', 'terbilang' => 'Four Hundred Fifteen Rupiah and Fifty Sen', 'jumlah_lampiran' => 0,
    ...
    public function test_manager_queue_contains_only_direct_reports_at_manager_stage(): void
    {
        ...
        $this->getJson('/api/v1/travel-reimbursements/approvals')->assertOk()
        ...
        $this->getJson('/api/v1/travel-reimbursements')->assertOk()->assertJsonCount(0, 'data');
    }
    ...
    public function test_manager_then_hr_approval_records_audit_and_prevents_replay(): void
    {
        ...
        $this->postJson($url.'/approve')->assertConflict();
        $this->getJson('/api/v1/travel-reimbursements/approvals')->assertJsonCount(0, 'data');
        $this->assertSame($manager->id, $record->fresh()->approved_by_manager);
        Sanctum::actingAs($hr);
        $this->getJson('/api/v1/travel-reimbursements/approvals')->assertOk()->assertJsonCount(1, 'data');
    ...
    public function test_unauthorized_users_cannot_list_or_decide(): void
    {
        ...
        $record = $this->permit($manager);
        $url = '/api/v1/travel-reimbursements/'.$record->id;
        $this->getJson('/api/v1/travel-reimbursements/approvals')->assertUnauthorized();
        ...
        foreach ([Role::User, Role::Admin, Role::Superadmin] as $role) {
            ...
            $this->getJson('/api/v1/travel-reimbursements/approvals')->assertForbidden();
    ...
    public function test_rejection_requires_reason_and_terminal_records_cannot_be_approved(): void
    {
        ...
        $this->postJson('/api/v1/travel-reimbursements/'.$cancelled->id.'/approve')->assertConflict();
    ...
    public function test_finance_supervisor_sees_both_stages_but_cannot_approve_own_request(): void
    {
        ...
        $this->getJson('/api/v1/travel-reimbursements/approvals')->assertOk()->assertJsonCount(2, 'data');
        $this->postJson('/api/v1/travel-reimbursements/'.$own->id.'/approve')->assertForbidden();
        $this->postJson('/api/v1/travel-reimbursements/'.$direct->id.'/approve', ['total' => '1.00'])->assertOk()
            ->assertJsonPath('data.status', 'Approved')->assertJsonPath('data.total', '415.50');
        $this->postJson('/api/v1/travel-reimbursements/'.$stageTwo->id.'/approve')->assertOk()
