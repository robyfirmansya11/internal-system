...
class ExpenseApprovalApiTest extends TestCase
{
    use RefreshDatabase;
    ...
    private function permit(User $manager): NotaPenggantianBiaya
    {
        ...
        $record = NotaPenggantianBiaya::create([
            ...
            'keterangan' => 'Business trip', 'jumlah_total' => '415.50', 'jumlah' => '415.50', 'jumlah_lampiran' => 0, 'terbilang' => 'Four Hundred Fifteen Rupiah and Fifty Sen', 'tanggal' => today(), 'informasi_transfer' => 'Bank transfer',
            'status' => 'Pending Approval', 'approval_level' => 1,
        ]);
        \App\Models\NotaPenggantianBiayaDetail::withoutEvents(fn () => $record->details()->createMany([
    ...
    public function test_manager_queue_contains_only_direct_reports_at_manager_stage(): void
    {
        ...
        $this->getJson('/api/v1/expense-reimbursements/approvals')->assertOk()
        ...
        $this->getJson('/api/v1/expense-reimbursements')->assertOk()->assertJsonCount(0, 'data');
    }
    ...
    public function test_manager_then_finance_approval_records_audit_and_prevents_replay(): void
    {
        ...
        $this->postJson($url.'/approve')->assertConflict();
        $this->getJson('/api/v1/expense-reimbursements/approvals')->assertJsonCount(0, 'data');
        $this->assertSame($manager->id, $record->fresh()->approved_by_manager);
        Sanctum::actingAs($hr);
        $this->getJson('/api/v1/expense-reimbursements/approvals')->assertOk()->assertJsonCount(1, 'data');
    ...
    public function test_unauthorized_users_cannot_list_or_decide(): void
    {
        ...
        $record = $this->permit($manager);
        $url = '/api/v1/expense-reimbursements/'.$record->id;
        $this->getJson('/api/v1/expense-reimbursements/approvals')->assertUnauthorized();
        ...
        foreach ([Role::User, Role::Admin, Role::Superadmin] as $role) {
            ...
            $this->getJson('/api/v1/expense-reimbursements/approvals')->assertForbidden();
    ...
    public function test_rejection_requires_reason_and_terminal_records_cannot_be_approved(): void
    {
        ...
        $this->postJson('/api/v1/expense-reimbursements/'.$cancelled->id.'/approve')->assertConflict();
    ...
    public function test_finance_supervisor_sees_both_stages_but_cannot_approve_own_request(): void
    {
        ...
        $this->getJson('/api/v1/expense-reimbursements/approvals')->assertOk()->assertJsonCount(2, 'data');
        $this->postJson('/api/v1/expense-reimbursements/'.$own->id.'/approve')->assertForbidden();
        $this->postJson('/api/v1/expense-reimbursements/'.$direct->id.'/approve', ['jumlah_total' => '1.00'])->assertOk()
            ->assertJsonPath('data.status', 'Approved')->assertJsonPath('data.jumlah_total', '415.50')
            ->assertJsonPath('data.details.0.jumlah', '200.25')
            ->assertJsonPath('data.details.1.jumlah', '215.25');
        $this->postJson('/api/v1/expense-reimbursements/'.$stageTwo->id.'/approve')->assertOk()
