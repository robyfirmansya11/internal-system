...
class LoanApprovalApiTest extends TestCase
{
    use RefreshDatabase;
    ...
    private function permit(User $manager): Kasbon
    {
        ...
        return Kasbon::create([
            ...
            'keterangan' => 'Business trip', 'jumlah_dana' => '415.50', 'terbilang' => 'Four Hundred Fifteen Rupiah and Fifty Sen', 'tanggal' => today(), 'informasi_transfer' => 'Bank transfer',
    ...
    public function test_manager_queue_contains_only_direct_reports_at_manager_stage(): void
    {
        ...
        $this->getJson('/api/v1/loan-notes/approvals')->assertOk()
        ...
        $this->getJson('/api/v1/loan-notes')->assertOk()->assertJsonCount(0, 'data');
    }
    ...
    public function test_manager_then_hr_approval_records_audit_and_prevents_replay(): void
    {
        ...
        $url = '/api/v1/loan-notes/'.$record->id;
        Sanctum::actingAs($hr);
        ...
        $this->postJson($url.'/approve')->assertConflict();
        $this->getJson('/api/v1/loan-notes/approvals')->assertJsonCount(0, 'data');
        $this->assertSame($manager->id, $record->fresh()->approved_by_manager);
        Sanctum::actingAs($hr);
        $this->getJson('/api/v1/loan-notes/approvals')->assertOk()->assertJsonCount(1, 'data');
    ...
    public function test_unauthorized_users_cannot_list_or_decide(): void
    {
        ...
        $record = $this->permit($manager);
        $url = '/api/v1/loan-notes/'.$record->id;
        $this->getJson('/api/v1/loan-notes/approvals')->assertUnauthorized();
        ...
        foreach ([Role::User, Role::Admin, Role::Superadmin] as $role) {
            ...
            $this->getJson('/api/v1/loan-notes/approvals')->assertForbidden();
    ...
    public function test_rejection_requires_reason_and_terminal_records_cannot_be_approved(): void
    {
        ...
        $this->postJson('/api/v1/loan-notes/'.$cancelled->id.'/approve')->assertConflict();
    ...
    public function test_finance_supervisor_sees_both_stages_but_cannot_approve_own_request(): void
    {
        ...
        $this->getJson('/api/v1/loan-notes/approvals')->assertOk()->assertJsonCount(2, 'data');
        $this->postJson('/api/v1/loan-notes/'.$own->id.'/approve')->assertForbidden();
        $this->postJson('/api/v1/loan-notes/'.$direct->id.'/approve', ['jumlah_dana' => '1.00'])->assertOk()
            ->assertJsonPath('data.status', 'Approved')->assertJsonPath('data.jumlah_dana', '415.50');
        $this->postJson('/api/v1/loan-notes/'.$stageTwo->id.'/approve')->assertOk()
