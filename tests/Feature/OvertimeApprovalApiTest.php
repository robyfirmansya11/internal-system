...
class OvertimeApprovalApiTest extends TestCase
{
    use RefreshDatabase;
    ...
    private function permit(User $manager): Lembur
    {
        ...
        return Lembur::create([
            ...
            'tanggal_lembur' => today(), 'bulan_lembur' => today()->format('Y-m'),
            'mulai_kerja' => '08:00', 'selesai_kerja' => '17:00', 'mulai_lembur' => '17:30', 'selesai_lembur' => '19:00',
            'jumlah_jam_lembur' => 1.5, 'uang_makan' => 25000, 'uraian_pekerjaan' => 'Overtime work',
    ...
    public function test_manager_queue_contains_only_direct_reports_at_manager_stage(): void
    {
        ...
        $this->getJson('/api/v1/overtime/approvals')->assertOk()
    ...
    public function test_manager_then_hr_approval_records_audit_and_prevents_replay(): void
    {
        ...
        $this->postJson($url.'/approve')->assertConflict();
        $this->getJson('/api/v1/overtime/approvals')->assertJsonCount(0, 'data');
        $this->assertSame($manager->id, $record->fresh()->approved_by_manager);
        Sanctum::actingAs($hr);
        $this->getJson('/api/v1/overtime/approvals')->assertOk()->assertJsonCount(1, 'data');
    ...
    public function test_unauthorized_users_cannot_list_or_decide(): void
    {
        ...
        $record = $this->permit($manager);
        $url = '/api/v1/overtime/'.$record->id;
        $this->getJson('/api/v1/overtime/approvals')->assertUnauthorized();
        ...
        foreach ([Role::User, Role::Admin, Role::Superadmin] as $role) {
            ...
            $this->getJson('/api/v1/overtime/approvals')->assertForbidden();
    ...
    public function test_rejection_requires_reason_and_terminal_records_cannot_be_approved(): void
    {
        ...
        $this->postJson('/api/v1/overtime/'.$cancelled->id.'/approve')->assertConflict();
        $this->assertSame('Cancelled', $cancelled->fresh()->status);
    }

    public function test_hr_applicant_is_finally_approved_by_direct_manager(): void
    {
        $manager = User::factory()->create(['level' => Role::Superuser, 'jabatan' => 'Manager']);
        $record = $this->permit($manager);
        $record->user->update(['level' => Role::Admin, 'jabatan' => 'HRD']);
        Sanctum::actingAs($manager);
        $this->postJson('/api/v1/overtime/'.$record->id.'/approve')->assertOk()
            ->assertJsonPath('data.status', 'Approved')->assertJsonPath('data.approval_level', 3)
            ->assertJsonPath('data.can_approve', false);
        $this->assertSame($manager->id, $record->fresh()->approved_by);
        $this->assertSame(1.5, (float) $record->fresh()->jumlah_jam_lembur);
        $this->assertSame(25000.0, (float) $record->fresh()->uang_makan);
    }
}
